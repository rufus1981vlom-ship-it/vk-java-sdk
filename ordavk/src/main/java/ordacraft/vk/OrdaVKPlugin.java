...
public class OrdaVKPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        VkMessageRouter router = new VkMessageRouter(config.settings(), vkApiClient, adminRegistry, ticketService, pendingReplyService, policy, console, governance, relay);

        poll = new VkLongPollService(router::onMessage);
        poll.start();

        var ticketsCommand = new TicketPlayerCommands(ticketService, config.settings().supportCooldown(), config.settings().supportMaxOpen(),
        ...
        getServer().getPluginManager().registerEvents(new JoinQuitListener(relay, pendingReplyService), this);
    }
